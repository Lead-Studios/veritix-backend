...
import {
  THROTTLE_ORGANIZER_KEY,
  THROTTLE_SKIP_KEY,
} from '../../src/decorators/throttle.decorator';
...
describe('OrganizerThrottleGuard', () => {
  ...
  describe('canActivate', () => {
    it('should return true when throttling is skipped', async () => {
      jest.spyOn(reflector, 'getAllAndOverride').mockReturnValueOnce(true); // THROTTLE_SKIP_KEY
    ...
    it('should extract organizer ID from JWT token', async () => {
      ...
      jest.spyOn(throttleService, 'checkThrottle').mockResolvedValue({
        limit: 100,
        remaining: 50,
      ...
      expect(result).toBe(true);
      expect(
        organizerService.getOrganizerSubscriptionPlan,
    ...
    it('should increment counter after successful check', async () => {
      ...
      expect(throttleService.incrementCounter).toHaveBeenCalledWith(
        'org_1',
        'day',
      );
    });
    ...
    it('should use custom limit key when specified', async () => {
      ...
      expect(throttleService.checkThrottle).toHaveBeenCalledWith(
        'org_1',
