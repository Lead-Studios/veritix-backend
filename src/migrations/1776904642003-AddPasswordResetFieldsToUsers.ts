...
export class AddPasswordResetFieldsToUsers1776904642003 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!hasColumn) {
            await queryRunner.query(`ALTER TABLE "users" ADD "passwordResetCode" varchar NULL`);
        }

        // Add passwordResetCodeExpiresAt column to users table if it doesn't exist
        hasColumn = await queryRunner.hasColumn("users", "passwordResetCodeExpiresAt");
        if (!hasColumn) {
            await queryRunner.query(`ALTER TABLE "users" ADD "passwordResetCodeExpiresAt" TIMESTAMP NULL`);
        }
    }
