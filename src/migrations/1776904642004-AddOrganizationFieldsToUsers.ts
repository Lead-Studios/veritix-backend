...
export class AddOrganizationFieldsToUsers1776904642004 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!hasColumn) {
            await queryRunner.query(`ALTER TABLE "users" ADD "organizationName" varchar NULL`);
        }
        ...
        if (!hasColumn) {
            await queryRunner.query(`ALTER TABLE "users" ADD "organizationWebsite" varchar NULL`);
        }
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE "users" DROP COLUMN "organizationWebsite"`);
        await queryRunner.query(`ALTER TABLE "users" DROP COLUMN "organizationName"`);
    }
}
