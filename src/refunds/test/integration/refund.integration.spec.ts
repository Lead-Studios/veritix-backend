...
import {
  Ticket,
  TicketStatus,
} from '../../../ticketing/entities/ticket.entity';
...
describe('Refund Integration Tests', () => {
  ...
  describe('End-to-End Refund Flow', () => {
    it('should complete full refund lifecycle', async () => {
      ...
      const refundResult = await refundService.createRefund({
        ...
        customerMessage:
          'We understand your situation and have processed a partial refund',
      });
    ...
    it('should process bulk refunds for event cancellation', async () => {
      ...
      expect(
        updatedTickets.every((t) => t.status === TicketStatus.CANCELLED),
      ).toBe(true);
      ...
      expect(
        refunds.every((r) => r.reason === RefundReason.EVENT_CANCELLED),
      ).toBe(true);
      expect(refunds.every((r) => r.status === RefundStatus.APPROVED)).toBe(
        true,
      );
    });
    ...
    it('should handle partial bulk refunds with processing fees', async () => {
      ...
      const bulkResult = await refundService.processBulkRefunds({
        ...
        customerMessage:
          'Event postponed. You can attend the new date or accept this partial refund.',
        autoProcess: true,
      });
      ...
      expect(
        updatedTickets.every((t) => t.status === TicketStatus.ACTIVE),
      ).toBe(true);
    });
    ...
    it('should handle refund cancellation', async () => {
      ...
      const cancelResult = await refundService.cancelRefund(
        refund.id,
        'user-123',
      );
  ...
  describe('Authorization and Security', () => {
    it('should enforce organizer authorization for event refunds', async () => {
      ...
      await expect(
        refundService.getRefundsByEvent(event.id, 'wrong-organizer'),
      ).rejects.toThrow(
      ...
      await expect(
        refundService.getRefundStats(event.id, 'wrong-organizer'),
      ).rejects.toThrow(
      ...
      await expect(
        refundService.processEventCancellationRefunds(
          event.id,
          'wrong-organizer',
        ),
      ).rejects.toThrow(
    ...
    it('should enforce refund cancellation authorization', async () => {
      ...
      await expect(
        refundService.cancelRefund(refund.id, 'unauthorized-user'),
      ).rejects.toThrow("You don't have permission to cancel this refund");
    });
  });
});
