...
export class AdminAuthService {
  ...
  async login(signInDto: SignInDto) {
    ...
    // 1. Retrieve user
    const user = await this.adminService.findOneByEmail(
  ...
  ): Promise<{ message: string; user: CreateAdminDto; token: string }> {
  ...
  async forgotPassword(email: string) {
    ...
    const resetToken =
      await this.generateTokenProvider.generatePasswordResetToken(admin);
    this.logger.log(
      `Password reset token generated for ${admin.email}: ${resetToken}`,
    );
  ...
  async resetPassword(token: string, passwordDto: ChangePasswordDto) {
    try {
      const { email } =
        await this.tokenProvider.verifyPasswordResetToken(token);
      ...
      const updatedUser = await this.adminService.updateAdminUser(admin.id, {
