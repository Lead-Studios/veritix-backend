...
export class SessionAnalyticsService {
  ...
  private getDateRange(filters: SessionAnalyticsFilterDto): {
    startDate: Date;
    endDate: Date;
  } {
    const now = new Date();
  ...
  private async getSessionAttendance(
    ...
    return results.map((result) => {
      ...
      const attendancePercentage =
        capacity > 0 ? (attendanceCount / capacity) * 100 : 0;
    ...
    return results.map((result) => ({
      ...
      averageAttendance:
        Number.parseInt(result.totalSessions) > 0
          ? Number.parseInt(result.totalAttendance) /
      ...
      punctualityPercentage:
        Number.parseFloat(result.punctualityPercentage) || 0,
    }));
  }
  ...
  private async getSpeakerAnalytics(
    ...
    return results.map((result) => {
      ...
      const totalSessionsWithTime =
        Number.parseInt(result.totalSessionsWithTime) || 0;
      const punctualityScore =
        totalSessionsWithTime > 0
          ? (onTimeSessions / totalSessionsWithTime) * 100
          : 0;
  ...
  private async getSessionOverlaps(
    ...
    return results.map((result) => {
      ...
      const averageAttendancePerSession =
        overlappingSessions > 0 ? totalAttendance / overlappingSessions : 0;
  ...
  private async getSummaryStats(
    ...
    if (filters.speaker) {
      sessionQuery.andWhere('session.speakerName = :speaker', {
        speaker: filters.speaker,
      });
    }
