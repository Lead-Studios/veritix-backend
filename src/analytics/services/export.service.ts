...
export class ExportService {
  ...
  private generateCsvContent(data: DashboardResponseDto): string {
    ...
    csv +=
      'Date,Total Attendees,Total Sessions,Average Attendance Per Session\n';
    ...
    csv +=
      'Session Title,Speaker,Track,Attendee Count,Capacity,Attendance Rate,Scheduled Start Time\n';
    ...
    csv +=
      'Speaker Name,Total Sessions,On Time Sessions,Late Sessions,Average Delay Minutes,Punctuality Rate\n';
    ...
    csv +=
      'Session Title,Speaker,Average Rating,Total Feedbacks,1 Star,2 Star,3 Star,4 Star,5 Star\n';
  ...
  private generatePdfContent(
    ...
    doc.text(
      `Average Feedback Score: ${data.summary.averageFeedbackScore.toFixed(2)}`,
    );
    doc.text(
      `Overall Punctuality Rate: ${data.summary.overallPunctualityRate.toFixed(2)}%`,
    );
    doc.moveDown();
    ...
    data.mostAttendedSessions.slice(0, 5).forEach((session, index) => {
      doc.text(
        `${index + 1}. ${session.sessionTitle} by ${session.speakerName}`,
      );
      doc.text(
        `   Attendees: ${session.attendeeCount}/${session.capacity} (${session.attendanceRate.toFixed(1)}%)`,
      );
      doc.moveDown(0.5);
    });
    ...
    data.speakerPunctuality.slice(0, 10).forEach((speaker, index) => {
      ...
      if (speaker.averageDelayMinutes > 0) {
        doc.text(
          `   Average Delay: ${speaker.averageDelayMinutes.toFixed(1)} minutes`,
        );
      }
      doc.moveDown(0.5);
    });
    ...
    if (data.feedbackStats.length > 0) {
      ...
      data.feedbackStats.slice(0, 10).forEach((feedback, index) => {
        doc.text(
          `${index + 1}. ${feedback.sessionTitle} by ${feedback.speakerName}`,
        );
        doc.text(
          `   Average Rating: ${feedback.averageRating.toFixed(2)}/5 (${feedback.totalFeedbacks} reviews)`,
        );
        doc.moveDown(0.5);
      });
    }
  ...
  async exportSessionAnalyticsToCsv(
    data: SessionAnalyticsDashboardDto,
    res: Response,
  ): Promise<void> {
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=session-analytics.csv',
    );
    res.send(csvContent);
  }

  async exportSessionAnalyticsToPdf(
    data: SessionAnalyticsDashboardDto,
    res: Response,
  ): Promise<void> {
    const doc = new PDFKit();
    ...
    res.setHeader(
      'Content-Disposition',
      'attachment; filename=session-analytics.pdf',
    );
  ...
  private generateSessionAnalyticsCsvContent(
    data: SessionAnalyticsDashboardDto,
  ): string {
    let csv = '';
    ...
    csv +=
      'Date,Total Sessions,Total Attendance,Average Attendance Per Session\n';
    ...
    csv +=
      'Session Title,Speaker,Track,Attendance Count,Capacity,Attendance Percentage,Average Rating,Feedback Count,Punctuality Score,Duration (min)\n';
    ...
    csv +=
      'Speaker Name,Total Sessions,Total Attendance,Average Attendance,Average Rating,Punctuality Score,Total Feedback\n';
    ...
    csv += 'TRACK ANALYTICS\n';
    csv +=
      'Track,Total Sessions,Total Attendance,Average Attendance,Average Rating,Punctuality Score\n';
    ...
    csv +=
      'Time Slot,Overlapping Sessions,Total Attendance,Average Attendance Per Session,Sessions\n';
  ...
  private generateSessionAnalyticsPdfContent(
    doc: PDFKit.PDFDocument,
    data: SessionAnalyticsDashboardDto,
  ): void {
    // Title
    ...
    doc
      .fontSize(14)
      .text(`Conference: ${data.conferenceName}`, { underline: true });
    doc.fontSize(12);
    doc.text(
    ...
    doc.text(
      `Overall Punctuality Score: ${data.overallPunctualityScore.toFixed(2)}%`,
    );
    doc.moveDown();
    ...
    data.topSessions.slice(0, 5).forEach((session, index) => {
      ...
      doc.text(
        `   Attendance: ${session.attendanceCount}/${session.capacity} (${session.attendancePercentage.toFixed(1)}%)`,
      );
      doc.text(
        `   Rating: ${session.averageRating.toFixed(2)}/5 (${session.feedbackCount} reviews)`,
      );
    ...
    data.speakerAnalytics.slice(0, 10).forEach((speaker, index) => {
      ...
      doc.text(
        `   Average Attendance: ${speaker.averageAttendance.toFixed(1)}`,
      );
    ...
    if (data.trackAnalytics.length > 0) {
      ...
      data.trackAnalytics.forEach((track, index) => {
        ...
        doc.text(
          `   Average Attendance: ${track.averageAttendance.toFixed(1)}`,
        );
    ...
    if (data.sessionOverlaps.length > 0) {
      ...
      data.sessionOverlaps.slice(0, 10).forEach((overlap, index) => {
        ...
        doc.text(
          `   Average per Session: ${overlap.averageAttendancePerSession.toFixed(1)}`,
        );
        doc.moveDown(0.5);
      });
    }
