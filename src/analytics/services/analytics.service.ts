...
export class AnalyticsService {
  ...
  async getDashboardData(
    ...
    const whereConditions = await this.buildWhereConditions(
      organizerId,
      filters,
    );
  ...
  private async buildWhereConditions(
    ...
    if (filters.startDate && filters.endDate) {
      conditions.scheduledStartTime = Between(
  ...
  private async getAttendancePerDay(
    whereConditions: any,
    filters: AnalyticsFilterDto,
    ...
    return results.map((result) => ({
      ...
      averageAttendancePerSession:
        Number.parseInt(result.totalSessions) > 0
          ? Number.parseInt(result.totalAttendees) /
            Number.parseInt(result.totalSessions)
          : 0,
    }));
  }
    ...
    return results.map((result) => ({
      ...
      attendanceRate:
        Number.parseInt(result.capacity) > 0
          ? (Number.parseInt(result.attendeeCount) /
              Number.parseInt(result.capacity)) *
            100
          : 0,
  ...
  private async getSpeakerPunctuality(
    ...
    return results.map((result) => {
      ...
      return {
        ...
        punctualityRate:
          totalSessions > 0 ? (onTimeSessions / totalSessions) * 100 : 0,
      };
    });
  }
  ...
  private async getSessionOverlaps(
    ...
    const query = this.sessionRepository
      ...
      .groupBy(
        'DATE(session.scheduledStartTime), TIME(session.scheduledStartTime), TIME(session.scheduledEndTime)',
      )
    ...
    return results.map((result) => {
      const overlappingSessions =
        Number.parseInt(result.overlappingSessions) || 0;
      ...
      return {
        ...
        averageAttendeesPerSession:
          overlappingSessions > 0 ? totalAttendees / overlappingSessions : 0,
      };
    });
  }
  ...
  private async getSummaryStats(
    ...
    const totalSessionsWithActualTime =
      Number.parseInt(punctualityResult.totalSessionsWithActualTime) || 0;
    const onTimeSessions =
      Number.parseInt(punctualityResult.onTimeSessions) || 0;
    ...
    return {
      ...
      averageFeedbackScore:
        Number.parseFloat(feedbackResult.averageFeedbackScore) || 0,
      overallPunctualityRate:
        totalSessionsWithActualTime > 0
          ? (onTimeSessions / totalSessionsWithActualTime) * 100
          : 0,
    };
  }
