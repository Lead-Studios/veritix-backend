...
describe("SessionAnalytics Integration", () => {
  ...
  beforeAll(async () => {
    ...
    conferenceRepository = moduleFixture.get<Repository<Conference>>(getRepositoryToken(Conference))
  ...
  describe("GET /analytics/sessions/dashboard/:organizerId", () => {
    ...
    it("should export to CSV", async () => {
      ...
      expect(response.headers["content-disposition"]).toContain("session-analytics.csv")
    })
  })
  ...
  describe("Data accuracy tests", () => {
    it("should calculate attendance correctly", async () => {
      ...
      const session1 = response.body.topSessions.find((s: any) => s.sessionId === "session-1")
      expect(session1.attendanceCount).toBe(2)

      // Session 2 has 1 attendee
      const session2 = response.body.topSessions.find((s: any) => s.sessionId === "session-2")
      expect(session2.attendanceCount).toBe(1)

      // Session 3 has 1 attendee
      const session3 = response.body.topSessions.find((s: any) => s.sessionId === "session-3")
