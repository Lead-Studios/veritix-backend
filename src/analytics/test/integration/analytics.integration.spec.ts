...
describe('Analytics Integration Tests', () => {
  ...
  describe('getDashboardData', () => {
    it('should return comprehensive analytics data', async () => {
      ...
      const speaker1Punctuality = result.speakerPunctuality.find(
        (sp) => sp.speakerName === 'Speaker 1',
      );
      expect(speaker1Punctuality.punctualityRate).toBe(0); // Late
      const speaker2Punctuality = result.speakerPunctuality.find(
        (sp) => sp.speakerName === 'Speaker 2',
      );
      expect(speaker2Punctuality.punctualityRate).toBe(100); // On time

      expect(result.feedbackStats).toHaveLength(2);
      const session1Feedback = result.feedbackStats.find(
        (fs) => fs.sessionTitle === 'Session 1',
      );
    ...
    it('should apply filters correctly', async () => {
      ...
      const conferenceFilterResult = await analyticsService.getDashboardData(
        organizerId,
        {
          conferenceId: conference1.id,
        },
      ...
      // Test track filter
      const trackFilterResult = await analyticsService.getDashboardData(
        organizerId,
        { track: 'Tech' },
      );
      ...
      // Test speaker filter
      const speakerFilterResult = await analyticsService.getDashboardData(
        organizerId,
  ...
  describe('getConferencesByOrganizer', () => {
    it('should return conferences for specific organizer', async () => {
      ...
      const result =
        await analyticsService.getConferencesByOrganizer(organizerId1);

      expect(result).toHaveLength(2);
      expect(result.every((conf) => conf.organizerId === organizerId1)).toBe(
        true,
      );
  ...
  describe('getTracksByConference', () => {
    it('should return unique tracks for conference', async () => {
      ...
      const result = await analyticsService.getTracksByConference(
        conference.id,
      );
  ...
  describe('getSpeakersByConference', () => {
    it('should return unique speakers for conference', async () => {
      ...
      const result = await analyticsService.getSpeakersByConference(
        conference.id,
      );
