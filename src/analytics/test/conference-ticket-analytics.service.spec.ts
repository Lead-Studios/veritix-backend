...
describe('ConferenceTicketAnalyticsService', () => {
  ...
  describe('getTotalTickets', () => {
    it('should return total tickets for a conference', async () => {
      ...
      expect(mockConferenceRepo.findOne).toHaveBeenCalledWith({ where: { id: 1 } });
    ...
    it('should throw NotFoundException when conference not found', async () => {
      ...
      await expect(service.getTotalTickets('999')).rejects.toThrow(NotFoundException);
      expect(mockConferenceRepo.findOne).toHaveBeenCalledWith({ where: { id: 999 } });
    });
  });
  ...
  describe('getTicketAnalytics', () => {
    ...
    it('should return analytics data without filter', async () => {
      ...
      expect(mockQueryBuilder.where).toHaveBeenCalledWith('ticket.conferenceId = :conferenceId', { conferenceId: '1' });
      expect(result).toEqual({
        conferenceId: '1',
        totalTickets: 3,
        filter: undefined,
    ...
    it('should throw NotFoundException when conference not found', async () => {
      ...
      await expect(service.getTicketAnalytics('999')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('exportTicketAnalytics', () => {
    ...
    it('should export data to CSV format', async () => {
      ...
      await service.exportTicketAnalytics('1', ExportFormat.CSV, undefined, mockResponse as Response);
    ...
    it('should export data to XLS format', async () => {
      ...
      await service.exportTicketAnalytics('1', ExportFormat.XLS, undefined, mockResponse as Response);
    ...
    it('should throw BadRequestException for unsupported format', async () => {
      await expect(
        service.exportTicketAnalytics('1', 'unsupported' as ExportFormat, undefined, mockResponse as Response)
    ...
    it('should throw NotFoundException when conference not found', async () => {
      ...
      await expect(
        service.exportTicketAnalytics('999', ExportFormat.CSV, undefined, mockResponse as Response)
    ...
    it('should export filtered data', async () => {
      ...
      await service.exportTicketAnalytics('1', ExportFormat.CSV, TimeFilter.DAILY, mockResponse as Response);
  ...
  describe('getDateRange', () => {
    it('should return correct date range for hourly filter', () => {
      ...
      expect(result.startDate.getTime()).toBeCloseTo(expectedStart.getTime(), -1000);
    ...
    it('should return correct date range for weekly filter', () => {
      ...
      const now = new Date();
      const expectedStart = new Date(now.getTime() - 4 * 7 * 24 * 60 * 60 * 1000);
    ...
    it('should return correct date range for monthly filter', () => {
      ...
      const now = new Date();
      const expectedStart = new Date(now.getTime() - 12 * 30 * 24 * 60 * 60 * 1000);
    ...
    it('should return correct date range for yearly filter', () => {
      ...
      const now = new Date();
      const expectedStart = new Date(now.getTime() - 5 * 365 * 24 * 60 * 60 * 1000);
  ...
  describe('getFilteredTicketData', () => {
    ...
    it('should call hourly aggregation when filter is hourly', async () => {
      const mockData = [{ timestamp: '2024-01-01T10:00:00', ticketCount: '1', conferenceId: '1' }];
      mockQueryBuilder.getRawMany.mockResolvedValue(mockData);

      const result = await service['getFilteredTicketData']('1', TimeFilter.HOURLY);

      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'DATE_TRUNC(\'hour\', ticket.purchaseDate) as timestamp',
        'COUNT(*) as ticketCount',
      ]);
      expect(mockQueryBuilder.groupBy).toHaveBeenCalledWith('DATE_TRUNC(\'hour\', ticket.purchaseDate)');
      expect(result).toEqual([
        { timestamp: '2024-01-01T10:00:00', ticketCount: 1, conferenceId: '1' },
      ]);
    });
    ...
    it('should call daily aggregation when filter is daily', async () => {
      const mockData = [{ timestamp: '2024-01-01', ticketCount: '2', conferenceId: '1' }];
      mockQueryBuilder.getRawMany.mockResolvedValue(mockData);

      const result = await service['getFilteredTicketData']('1', TimeFilter.DAILY);

      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'DATE_TRUNC(\'day\', ticket.purchaseDate) as timestamp',
        'COUNT(*) as ticketCount',
      ]);
      expect(mockQueryBuilder.groupBy).toHaveBeenCalledWith('DATE_TRUNC(\'day\', ticket.purchaseDate)');
      expect(result).toEqual([
        { timestamp: '2024-01-01', ticketCount: 2, conferenceId: '1' },
      ]);
    });
    ...
    it('should call weekly aggregation when filter is weekly', async () => {
      const mockData = [{ timestamp: '2024-01-01', ticketCount: '3', conferenceId: '1' }];
      mockQueryBuilder.getRawMany.mockResolvedValue(mockData);

      const result = await service['getFilteredTicketData']('1', TimeFilter.WEEKLY);

      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'DATE_TRUNC(\'week\', ticket.purchaseDate) as timestamp',
        'COUNT(*) as ticketCount',
      ]);
      expect(mockQueryBuilder.groupBy).toHaveBeenCalledWith('DATE_TRUNC(\'week\', ticket.purchaseDate)');
      expect(result).toEqual([
        { timestamp: '2024-01-01', ticketCount: 3, conferenceId: '1' },
      ]);
    });
    ...
    it('should call monthly aggregation when filter is monthly', async () => {
      ...
      const result = await service['getFilteredTicketData']('1', TimeFilter.MONTHLY);

      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'DATE_TRUNC(\'month\', ticket.purchaseDate) as timestamp',
        'COUNT(*) as ticketCount',
      ]);
      expect(mockQueryBuilder.groupBy).toHaveBeenCalledWith('DATE_TRUNC(\'month\', ticket.purchaseDate)');
      expect(result).toEqual([
        { timestamp: '2024-01-01', ticketCount: 3, conferenceId: '1' },
      ]);
    });
    ...
    it('should call yearly aggregation when filter is yearly', async () => {
      ...
      const result = await service['getFilteredTicketData']('1', TimeFilter.YEARLY);

      expect(mockQueryBuilder.select).toHaveBeenCalledWith([
        'DATE_TRUNC(\'year\', ticket.purchaseDate) as timestamp',
        'COUNT(*) as ticketCount',
      ]);
      expect(mockQueryBuilder.groupBy).toHaveBeenCalledWith('DATE_TRUNC(\'year\', ticket.purchaseDate)');
      expect(result).toEqual([
        { timestamp: '2024-01-01', ticketCount: 3, conferenceId: '1' },
      ]);
    });
