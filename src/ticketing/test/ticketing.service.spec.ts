...
describe('TicketingService', () => {
  ...
  describe('purchaseTickets', () => {
    ...
    it('should throw NotFoundException for non-existent event', async () => {
      ...
      await expect(service.purchaseTickets(purchaseData)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('getTicket', () => {
    ...
    it('should throw NotFoundException for non-existent ticket', async () => {
      ...
      await expect(service.getTicket('non-existent')).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe('cancelTicket', () => {
    ...
    it('should throw ForbiddenException for unauthorized user', async () => {
      ...
      await expect(
        service.cancelTicket('ticket-1', 'unauthorized-user'),
    ...
    it('should throw BadRequestException for already used ticket', async () => {
      ...
      await expect(service.cancelTicket('ticket-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('getEventStats', () => {
    ...
    it('should throw ForbiddenException for unauthorized organizer', async () => {
      ...
      await expect(
        service.getEventStats('event-1', 'wrong-organizer'),
      ).rejects.toThrow(ForbiddenException);
    });
  });
});
