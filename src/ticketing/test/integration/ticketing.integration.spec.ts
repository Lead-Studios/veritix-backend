...
describe('Ticketing Integration Tests', () => {
  ...
  describe('End-to-End Ticket Flow', () => {
    it('should complete full ticket lifecycle', async () => {
      ...
      const qrVerification = qrCodeService.verifyQrCode(
      ...
      const qrDataVerification = qrCodeService.verifyQrCode(
        storedTicket!.qrCodeData,
      );
      ...
      const stats = await ticketingService.getEventStats(
        event.id,
        'organizer-123',
      );
    ...
    it('should handle ticket cancellation correctly', async () => {
      ...
      const cancelResult = await ticketingService.cancelTicket(
        ticketId,
        'user-123',
      );
