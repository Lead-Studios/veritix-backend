...
export class TicketTransferService {
  ...
  async create(userId: string, createTransferDto: CreateTransferDto) {
    ...
    if (existingTransfer) {
      throw new HttpException(
        "There is already a pending transfer for this ticket",
        HttpStatus.CONFLICT,
      );
    }
    ...
    if (createTransferDto.recipientId) {
      ...
      const recipient = await this.usersService.GetOneByEmail(
        createTransferDto.recipientEmail,
      );
      if (recipient) {
    ...
    if (transfer.recipientId) {
      ...
      console.log(
        `Email notification would be sent to ${transfer.recipientEmail}`,
      );
    }
  ...
  async updateStatus(
    ...
    if (transfer.status !== TransferStatus.PENDING) {
      throw new HttpException(
        `Cannot ${status} a non-pending transfer`,
        HttpStatus.BAD_REQUEST,
      );
    }
    ...
    if (status === TransferStatus.ACCEPTED && transfer.recipientId !== userId) {
      throw new HttpException(
        "Only the recipient can accept a transfer",
        HttpStatus.FORBIDDEN,
      );
    }
    ...
    if (status === TransferStatus.REJECTED && transfer.recipientId !== userId) {
      throw new HttpException(
        "Only the recipient can reject a transfer",
        HttpStatus.FORBIDDEN,
      );
    }
    ...
    if (status === TransferStatus.ACCEPTED && verificationCode) {
      ...
      if (
        transfer.verificationExpiry &&
        new Date() > transfer.verificationExpiry
      ) {
        throw new HttpException(
    ...
    if (status === TransferStatus.ACCEPTED) {
      await this.ticketsService.updateOwner(
        transfer.ticketId,
    ...
    const notificationRecipientId =
      userId === transfer.senderId ? transfer.recipientId : transfer.senderId;
  ...
  async generateVerificationCode(id: string, userId: string) {
    ...
    if (transfer.senderId !== userId) {
      throw new HttpException(
        "Only the sender can generate verification code",
        HttpStatus.FORBIDDEN,
      );
    }
    ...
    if (transfer.status !== TransferStatus.PENDING) {
      throw new HttpException(
        "Can only generate verification code for pending transfers",
        HttpStatus.BAD_REQUEST,
      );
    }
  ...
  async completeTransfer(id: string, userId: string, verificationCode: string) {
    ...
    if (transfer.recipientId !== userId) {
      throw new HttpException(
        "Only the recipient can complete the transfer",
        HttpStatus.FORBIDDEN,
      );
    }
    ...
    if (transfer.status !== TransferStatus.ACCEPTED) {
      throw new HttpException(
        "Can only complete accepted transfers",
        HttpStatus.BAD_REQUEST,
      );
    }
