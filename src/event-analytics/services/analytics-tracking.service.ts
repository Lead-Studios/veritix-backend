...
export class AnalyticsTrackingService {
  ...
  async trackView(
    ...
    const eventView = this.eventViewRepository.create({
      ...
      trafficSource: this.normalizeTrafficSource(
        trackViewDto.trafficSource,
        trackViewDto.referrerUrl,
      ),
    });
   ...
   */
  async trackPurchase(
    trackPurchaseDto: TrackPurchaseDto,
    ipAddress?: string,
  ): Promise<void> {
    const purchaseLog = this.purchaseLogRepository.create({
      ...
      trafficSource: this.normalizeTrafficSource(
        trackPurchaseDto.trafficSource,
        trackPurchaseDto.referrerUrl,
      ),
      completedAt:
        trackPurchaseDto.status === 'completed' ? new Date() : undefined,
    });
    ...
    if (trackPurchaseDto.status === 'completed') {
      await this.updateViewConversion(
        trackPurchaseDto.eventId,
        trackPurchaseDto.purchaserId,
        purchaseLog.id,
      );
    }
  }
  ...
  async trackEngagement(
    trackEngagementDto: TrackEngagementDto,
    ipAddress?: string,
  ): Promise<void> {
    const engagement = this.eventEngagementRepository.create({
      ...
      ipAddress,
      trafficSource: this.normalizeTrafficSource(
        trackEngagementDto.trafficSource,
      ),
    });
   ...
   */
  private async updateViewConversion(
    eventId: string,
    userId: string,
    purchaseId: string,
  ): Promise<void> {
