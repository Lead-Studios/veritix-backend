...
describe('EventAnalyticsService', () => {
  ...
  describe('getEventAnalytics', () => {
    ...
    beforeEach(() => {
      ...
      mockPurchaseLogRepository.createQueryBuilder.mockReturnValue(
        mockQueryBuilder,
      );
      mockEventViewRepository.createQueryBuilder.mockReturnValue(
        mockQueryBuilder,
      );
    ...
    it('should throw ForbiddenException for unauthorized organizer', async () => {
      ...
      await expect(
        service.getEventAnalytics('event-123', 'wrong-organizer', filters),
    ...
    it('should apply traffic source filter', async () => {
      ...
      await service.getEventAnalytics(
        'event-123',
        'organizer-123',
        filtersWithSource,
      );
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        'purchase.trafficSource = :trafficSource',
        {
    ...
    it('should exclude refunded purchases by default', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith(
        'purchase.status != :refundedStatus',
        {
  ...
  describe('date range handling', () => {
    ...
    it('should default to last 30 days when no dates provided', () => {
      ...
      expect(result.end.getTime() - result.start.getTime()).toBeCloseTo(
        30 * 24 * 60 * 60 * 1000,
        -1000,
      ); // ~30 days
    });
