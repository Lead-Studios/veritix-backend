...
describe('AuthService', () => {
  ...
  describe('createUser', () => {
    it('should throw 409 if email already exists', async () => {
      ...
      await expect(
        service.createUser({ email: 'test@mail.com', password: 'Password123!' }),
      ).rejects.toThrow(ConflictException);
    });

    it('should throw 409 if password invalid', async () => {
      mockUserRepository.findOne.mockResolvedValue(null);
      mockUserHelper.validatePassword.mockReturnValue(false);

      await expect(
        service.createUser({ email: 'test@mail.com', password: 'weak' }),
      ).rejects.toThrow(ConflictException);
    });

    it('should create user and send email', async () => {
      mockUserRepository.findOne.mockResolvedValue(null);
      mockUserHelper.validatePassword.mockReturnValue(true);
      mockUserHelper.hashPassword.mockResolvedValue('hashed');
      mockUserHelper.generateOtp.mockReturnValue('123456');
  ...
  describe('verifyOtp', () => {
    it('should throw 401 if user not found', async () => {
      ...
      await expect(
        service.verifyOtp('email@test.com', '123456'),
    ...
    it('should throw 401 if OTP expired', async () => {
      mockUserRepository.findOne.mockResolvedValue({
        otp: '123456',
        otpExpiresAt: new Date(),
      });

      mockUserHelper.isOtpExpired.mockReturnValue(true);
    ...
    it('should verify OTP and return tokens', async () => {
      ...
      mockUserHelper.isOtpExpired.mockReturnValue(false);

      mockJwtHelper.generateAccessToken.mockReturnValue('access');
      mockJwtHelper.generateRefreshToken.mockReturnValue('refresh');
  ...
  // =========================
  describe('login', () => {
    it('should throw 401 for wrong credentials', async () => {
      mockUserRepository.findOne.mockResolvedValue(null);

      await expect(
        service.login('email@test.com', 'wrong'),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('should resend OTP if user not verified', async () => {
      const user = {
        id: 1,
        isVerified: false,
        email: 'test@mail.com',
      ...
      mockUserRepository.findOne.mockResolvedValue(user);
      mockUserHelper.comparePassword.mockResolvedValue(true);
      mockUserHelper.generateOtp.mockReturnValue('123456');

      const result = await service.login('test@mail.com', 'Password123!');
  ...
  describe('refreshToken', () => {
    ...
    it('should return new access token', async () => {
      mockJwtHelper.verifyRefreshToken.mockReturnValue({ sub: 1 });

      mockJwtHelper.generateAccessToken.mockReturnValue('new-access');
  ...
  describe('requestResetPasswordOtp', () => {
    it('should throw 404 if user not found', async () => {
      ...
      await expect(
        service.requestResetPasswordOtp('test@mail.com'),
      ).rejects.toThrow(NotFoundException);
    });
    ...
    it('should send reset OTP email', async () => {
      ...
      await service.requestResetPasswordOtp('test@mail.com');
  ...
  describe('resetPassword', () => {
    it('should throw 401 if OTP expired', async () => {
      ...
      await expect(
        service.resetPassword('test@mail.com', '123456', 'Password123!', 'Password123!'),
      ).rejects.toThrow(UnauthorizedException);
    });

    it('should throw 400 if passwords do not match', async () => {
      await expect(
        service.resetPassword('test@mail.com', '123456', 'a', 'b'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw 400 for invalid password format', async () => {
      mockUserRepository.findOne.mockResolvedValue({ otp: '123456' });
      mockUserHelper.validatePassword.mockReturnValue(false);

      await expect(
        service.resetPassword('test@mail.com', '123456', 'weak', 'weak'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should reset password successfully', async () => {
      const user = { id: 1, otp: '123456' };
